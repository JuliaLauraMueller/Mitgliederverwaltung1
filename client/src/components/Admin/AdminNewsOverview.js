...
class AdminNewsOverview extends Component {
  constructor(props) {
    super(props);
    ...
    this.getNewsRows = this.getNewsRows.bind(this);
    this.collapseNews = this.collapseNews.bind(this);
        ...
        <Row>
          <Col sm="12">
              ...
              <Col sm="12">
                <div>
                  <input
                    ...
                    color="primary"
                    onClick={this.collapseNews}
                    value="News hinzufügen"
                  />
                  <Collapse isOpen={this.state.collapseNews}>
                    <Card>
                      <CardBody>
                        <AdminCrateNews close={this.collapseNews} />
                      </CardBody>
                    </Card>
                  </Collapse>
                </div>
            ...
            <Table hover className="adminTable">
              ...
              <tbody>{this.getNewsRows(this.props.news)}</tbody>
            </Table>
          </Col>
        </Row>
      </div>
  ...
  getNewsRows(news) {
    return news.map(news => {
      let EditButton = {};
      let DeleteButton = {};
      var tooltipIdEdit = 'tooltip-edit-' + news._id;
      var tooltipIdDelete = 'tooltip-delete-' + news._id;
      EditButton = (
        <span
      ...
      return (
        <tr key={news._id}>
          <td>{news.title}</td>
          <td>{news.date}</td>
          <td className="d-none d-md-table-cell">{news.author.surname}</td>
          <td>
            {EditButton}
            {DeleteButton}
          </td>
  ...
  collapseNews() {
    this.setState(state => ({ collapseNews: !state.collapseNews }));
  }
}
...
export default connect(mapStateToProps)(AdminNewsOverview);
